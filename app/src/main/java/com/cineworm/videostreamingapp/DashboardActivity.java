...
import com.cineworm.adapter.TransactionAdapter;
...
public class DashboardActivity extends BaseActivity {
    ...
    TextView textCurrentPlan, textExpiresOn, textChangePlan,tvUserTransaction;
    TextView textLsDate, textLsPlan, textLsAmount;
    ItemDashBoard itemDashBoard;
    MyApplication myApplication;

    RecyclerView rvTransaction;
    ArrayList<ItemTransaction> transactionList;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        tvUserTransaction = findViewById(R.id.tvUserTransaction);
        rvTransaction = findViewById(R.id.rvTransaction);
        transactionList = new ArrayList<>();
        rvTransaction.setHasFixedSize(true);
        rvTransaction.setLayoutManager(new LinearLayoutManager(DashboardActivity.this, LinearLayoutManager.VERTICAL, false));
        rvTransaction.setFocusable(false);
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                try {
                    ...
                    if (jsonArray.length() > 0) {
                        ...
                        JSONArray trArray = objJson.getJSONArray("transactions_list");
                        for (int i = 0; i < trArray.length(); i++) {
                            JSONObject joTr = trArray.getJSONObject(i);
                            ItemTransaction objItem = new ItemTransaction();
                            objItem.setPlanName(joTr.getString("plan_name"));
                            objItem.setPlanAmount(joTr.getString("amount"));
                            objItem.setPaymentGateway(joTr.getString("payment_gateway"));
                            objItem.setPaymentId(joTr.getString("payment_id"));
                            objItem.setPaymentDate(joTr.getString("payment_date"));
                            objItem.setPaymentCurrency(joTr.getString("currency_code"));
                            transactionList.add(objItem);
                        }
    ...
    private void displayData() {
        ...
        if (transactionList.isEmpty()) {
            ...
            tvUserTransaction.setVisibility(View.GONE);
        } else {
            transactionAdapter = new TransactionAdapter(DashboardActivity.this, transactionList);
