...
public class MovieDetailsActivity extends BaseActivity {
    ...
    MaterialButton btnDownload, btnWatchList, btnTrailer;
    private Casty casty;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            ...
            Log.e("MovieDetails", "Failed to initialize Cast: " + e.getMessage());
            e.printStackTrace();
            // Hide cast button if initialization fails
            mediaRouteButton = findViewById(R.id.media_route_button);
        ...
        rvRelated.setLayoutManager(new LinearLayoutPagerManager(MovieDetailsActivity.this, LinearLayoutManager.HORIZONTAL, false, 1));
    ...
    private void displayData() {
        ...
        if (itemMovie.getMovieType().equals(Constant.VIDEO_TYPE_HLS) && !itemMovie.getMovieUrl().isEmpty()) {
            if (PlayerUtil.isYoutubeUrl(itemMovie.getMovieUrl())) {
                itemMovie.setMovieType(Constant.VIDEO_TYPE_YOUTUBE);
            } else if (PlayerUtil.isVimeoUrl(itemMovie.getMovieUrl())) {
                itemMovie.setMovieType(Constant.VIDEO_TYPE_VIMEO);
            }
        }
    ...
    private void initTrailerPlayer() {
        if (itemMovie.getMovieTrailer().isEmpty()) {
            ...
            btnTrailer.setOnClickListener(view -> {
                String streamUrl = itemMovie.getMovieTrailer();
                Intent intent = getIntent();
                if (PlayerUtil.isYoutubeUrl(streamUrl)) {
                    intent.setClass(MovieDetailsActivity.this, YtPlayerActivity.class);
                    intent.putExtra("videoId", PlayerUtil.getVideoIdFromYoutubeUrl(streamUrl));
                } else if (PlayerUtil.isVimeoUrl(streamUrl)) {
                    intent.setClass(MovieDetailsActivity.this, VimeoPlayerActivity.class);
                    intent.putExtra("videoId", PlayerUtil.getVideoIdFromVimeoUrl(streamUrl));
                } else {
                    intent.setClass(MovieDetailsActivity.this, TrailerExoPlayerActivity.class);
    ...
    private void setPlayer() {
        if (itemMovie.getMovieUrl().isEmpty() || isUpcoming) {
            ...
            Log.d("MovieDetails", "Movie Type: " + itemMovie.getMovieType());
            Log.d("MovieDetails", "Movie URL: " + itemMovie.getMovieUrl());
            switch (itemMovie.getMovieType()) { //URL Embed
                ...
                case Constant.VIDEO_TYPE_HLS:
                    if (casty != null && casty.isConnected()) {
                        ...
                        try {
                            ItemPlayer playerData = getPlayerData();
                            if (playerData == null || playerData.getDefaultUrl() == null || playerData.getDefaultUrl().isEmpty()) {
                                Log.e("MovieDetails", "Player data is null or invalid!");
                                showToast("Unable to play video. Invalid URL.");
                                return;
                            }
                            ...
                            Log.e("MovieDetails", "Error creating player: " + e.getMessage());
                            e.printStackTrace();
    ...
    private void playViaCast() {
        if (itemMovie.getMovieType().equals(Constant.VIDEO_TYPE_LOCAL) || itemMovie.getMovieType().equals(Constant.VIDEO_TYPE_URL)
                || itemMovie.getMovieType().equals(Constant.VIDEO_TYPE_HLS)) {
