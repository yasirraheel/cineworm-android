...
import com.cineworm.adapter.HomeSportAdapter;
...
public class SportDetailsActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        try {
            ...
            Log.e("SportDetails", "Failed to initialize Cast: " + e.getMessage());
            e.printStackTrace();
            mediaRouteButton = findViewById(R.id.media_route_button);
        ...
        rvRelated.setLayoutManager(new LinearLayoutPagerManager(SportDetailsActivity.this, LinearLayoutManager.HORIZONTAL, false, 2));
    ...
    private void displayData() {
        ...
        if (itemSport.getSportType().equals(Constant.VIDEO_TYPE_HLS) && !itemSport.getSportUrl().isEmpty()) {
            if (PlayerUtil.isYoutubeUrl(itemSport.getSportUrl())) {
                itemSport.setSportType(Constant.VIDEO_TYPE_YOUTUBE);
            } else if (PlayerUtil.isVimeoUrl(itemSport.getSportUrl())) {
                itemSport.setSportType(Constant.VIDEO_TYPE_VIMEO);
            }
        }
    ...
    private void playViaCast() {
        if (itemSport.getSportType().equals(Constant.VIDEO_TYPE_LOCAL) || itemSport.getSportType().equals(Constant.VIDEO_TYPE_URL) || itemSport.getSportType().equals(Constant.VIDEO_TYPE_HLS)) {
